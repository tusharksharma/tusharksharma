...
const regex = /id:\s*(\d+),[\s\S]*?title:\s*"([^"]+)"[\s\S]*?macros:\s*\{\s*protein:\s*(\d+),\s*calories:\s*(\d+),\s*fat:\s*(\d+),\s*carbs:\s*(\d+),\s*netCarbs:\s*\d+,\s*estimated:\s*(true|false)/g;
...
while ((match = regex.exec(src)) !== null) {
  const [, id, title, p, cal, f, c, est] = match;
