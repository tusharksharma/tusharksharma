      ...
      <div className="max-w-3xl mx-auto px-4 py-16">
        ...
        <div className="text-center mb-10">
          ...
          <div className="flex gap-3 justify-center mt-5">
            <a href="#timeline" className="px-5 py-2.5 bg-amber-500 text-black font-bold rounded-xl text-xs hover:bg-amber-400 transition-colors cursor-pointer">
              Start This Week
            </a>
            <a href="#grocery" className="px-5 py-2.5 bg-neutral-800 text-white font-bold rounded-xl border border-neutral-700 text-xs hover:bg-neutral-700 transition-colors cursor-pointer">
              Shop First
            </a>
          </div>
        </div>
        ...
        <div className="space-y-3" id="timeline">
          {TIMELINE.map((entry, i) => {
            if (entry.type === "cook") {
              return <CookDay key={i} {...entry} />;
            }
            if (entry.type === "leftover") {
              return <LeftoverDay key={i} day={entry.day} meal={entry.meal} />;
            }
        ...
        <div className="mt-8 text-center py-6 bg-neutral-900/30 border border-neutral-800 rounded-xl">
          <p className="text-neutral-500 text-xs">
            Once you've shopped and checked everything off:
          </p>
          <p className="text-amber-400 font-bold text-sm mt-1">
            Your entire week is handled. Zero food decisions left.
          </p>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-neutral-600 text-xs">
            Next week drops Sunday &middot; Same system, new flavors &middot; Zero thinking
          </p>
        </div>
      </div>
    </section>
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-4">
            ...
            <div className="mt-2 flex gap-1.5 flex-wrap">
              {needs.map((n) => (
                <span key={n} className="text-[10px] bg-neutral-800 text-neutral-500 px-2 py-0.5 rounded-full">
                  {n}
                </span>
              ))}
            </div>
